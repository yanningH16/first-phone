  ...
  <div class="getprice_box">
    <scroll ref="scroll" class="scroll" :click="false">
      <div class="stepOne_box">
        ...
        <div class="step10">
          <h2 v-if="type==1 || type==2 || type==3">十、填写付款订单信息</h2>
          <h2 v-else>九、填写付款订单信息</h2>
          <div class="warn onlyAllow">
            <p>
              1. 若平台包邮店铺不包邮，填写实付金额，会一起退还<br>
              2. 若淘宝价格与任务价格不符，核对商品正确请直接下单，提交实际付款价格，无需联系客服<br>
              3. 下单后严禁擅自淘宝退货/退款，一经发现将永久封号处理！<br>
              4. 白拿商品不接受指定快递
            </p>
          </div>
          <div class="bLine">
        ...
        <div class="step11">
          <h2 v-if="type==1 || type==2 || type==3">十一、上传订单详情截图</h2>
          <h2 v-else>十、上传订单详情截图</h2>
          <div>
        ...
        <div class="warn">
          <p>
            提示：<br>
            1. 严禁转售中奖白拿商品，一经发现，直接封号<br>
            2. 商品没有质量问题，一律不予退换货<br>
            3. 商品存在质量问题，请联系在线客服<br>
            4. 商家将在72小时内操作发货，请耐心等待，不要到旺旺上催商家
          </p>
        </div>
        <div class="next">
          <!--<router-link :to="{ name: 'submitSuccess', params: { type: 'price' } }">
            <button :disabled="!isOk" :class="{ook: !isOk}">提交付款订单信息</button>
          </router-link>-->
          <button :disabled="!isOk" :class="{ook: !isOk}" @click="doNext">提交付款订单信息</button>
        </div>
      </div>
    </scroll>
  </div>
