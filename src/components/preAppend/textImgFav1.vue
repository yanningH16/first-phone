    ...
    <div class="stepOne_box">
      ...
      <div class="step2">
        ...
        <div class="warn">
          <p>
            提示：<br>
            1. 请按照好评要求进行评价，请勿直接去淘宝评价，好评须商家审核通过后才可以评价到淘宝/天猫，否则降低中奖率<br>
            2. 不得给商家低于5星好评，否则平台将会冻结账户，账户金额不予提现<br>
            3. 若淘宝/天猫快递未签收，就提前在平台或淘宝店铺确认好评，平台将扣除当此任务本金作为处罚<br>
            4. 严禁到旺旺上催促商家审核评价，或提及"平台"，"免费白拿"等关键词，否则封号
          </p>
        </div>
        <div class="next">
            <button :disabled="!isOk" :class="{ook: !isOk}" @click="next">下一步</button>
        </div>
      </div>
    </scroll>
  </div>
    ...
    methods: {
      next(){
        ...
        if (this.text!==''&& this.goodsImg.length>0) {
          this.$router.push({ name:'preAppendTextImgFav2',query:{text:this.text,goodImg:this.goodImg, buyerTaskRecordId: this.$route.query.buyerTaskRecordId} })
        }else{
          this.$vux.alert.show({
            title:'错误提示',
            content: '内容与图片不能为空',
