<template>
  <div class="app">
    <div class="wrap" v-for="(item,index) in goods" :key="index" @click="doRouter(index,item.sellerTaskId)">
      ...
      <h4><span v-if="hot===1" class="hot iconfont icon-hot" alt=""></span>{{item.info}}</h4>
      ...
      <div class="btn">
        ...
        <p class="zhuan"  v-if="item.msg.state === 1">白拿还赚<span>{{item.msg.zhuan}}</span>金币</p>
        <p class="hua"  v-if="item.msg.state === 2">花<span>{{item.msg.hua}}</span>金币白拿</p>
      </div>
    </div>
  </div>
</template>
